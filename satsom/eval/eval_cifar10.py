...
def eval_som(
    ...
    """
    Train SatSOM, DSDM, PROPRE on CIFAR10 Embeddings.
    """
    ...
    # Logging
    logger = logging.getLogger("eval_cifar10")
    ...
    # Load Data (Features)
    # ----------------------------------------------
    logger.info("Loading CIFAR10 Features...")

    dataset_dir = dataset_root_dir or (out_dir / "data")

    # Extract features using ResNet
    # We do this on 'device' but return to CPU to organize splits
    features, labels = extract_cifar_features(str(dataset_dir), device=device)

    # Move to device for training loop (unless dataset is huge, then keep on CPU)
    # CIFAR10 embeddings (60k, 512) is ~120MB, fits easily on GPU.
    ...
    # Group samples by class for custom split logic
    ...
    for lbl in unique_labels:
        ...
        # We respect the train_perc based on the TOTAL available data (Train+Test combined)
        # as per previous script logic
        n_train = int(train_perc * len(imgs_lbl))

        by_label["train"][lbl] = imgs_lbl[:n_train]
        by_label["test"][lbl] = imgs_lbl[n_train:]

    # default phasing: each class alone
    if phases is None:
    ...
    for phase_idx, phase_labels in enumerate(phases, start=1):
        ...
        for i, (img, lbl) in train_iter:
            ...
            # We are using 512-dim vectors, not images.
            # create_satsom_image cannot visualize this meaningfully.
    ...
    df.to_csv(out_dir / "accuracy_cifar10_resnet.csv", index=False)
    logger.info("Saved accuracy_cifar10_resnet.csv")
